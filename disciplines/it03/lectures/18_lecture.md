...
::: info
`FLOAT` и `DOUBLE` существуют потому, что в реальных задачах часто нужны **очень большие или очень маленькие числа**, но при этом не критична идеальная точность. Эти типы хранят число в формате с плавающей точкой (аналогично научной записи числа):

$$
\text{число} \approx \text{знак} \times \text{мантисса} \times \text{основание}^{\text{порядок}}
$$

Мантисса — это значимая часть числа. Обычно основание — это `2`, потому что числа хранятся в двоичном виде.

Пример записи числа (смысл, а не точные биты хранения):

1) Сначала переводим число в двоичный вид: `13 = 1101`. Дробная часть `0.25` — это `1/4`, а дробные разряды в двоичной системе идут как `1/2, 1/4, 1/8, ...`. Поэтому `0.01 = 0×(1/2) + 1×(1/4) = 1/4`.
...
2) Затем нормализуем запись (переносим точку влево, чтобы мантисса была в диапазоне `1.xxx`). Сдвиг точки на 3 позиции влево означает умножение на `2^3`:

   $$
   13.25_{10} \approx {+1.101}_{2} \times 2^{3}
   $$

Из‑за такого формата записи часть цифр округляется — поэтому и появляется погрешность.
:::
