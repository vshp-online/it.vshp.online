...
Обратите внимание: сравнение `column = NULL` **не работает**, потому что `NULL` не считается ни равным, ни неравным чему-либо.
...
**Результат:**
Три сотрудника из отдела `IT` с должностями, содержащими «программист», у кого либо либо есть корпоративная почта на домене `@company.ru`, либо её нет; отсортированы по убыванию зарплаты (при равенстве — по фамилии)
