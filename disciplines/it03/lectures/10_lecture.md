...
::: note
Движок СУБД вычисляет вложенные запросы от глубины к поверхности: сначала выполняется самый глубокий уровень, затем его результат передаётся подзапросу уровня 1, и только после этого формируется внешний запрос. Такой порядок помогает понимать, какие данные доступны на каждой ступени.
:::
...
::: info
Поле `effort` хранит субъективную оценку нагрузки (low / medium / high). Мы не будем делать из него отдельный справочник, но будем использовать в фильтрах и подзапросах.
:::
