...
1. **Разработать хранимую процедуру `book_room`**:
   ...
   - **Логика работы:**
     ...
     - Проверяет, доступен ли номер для бронирования (статус `available`).
       - Если номер уже забронирован (статус `booked`), возвращает варнинг (проверять другие статусы не обязательно из-за `ENUM`):

         ```sql
         SIGNAL SQLSTATE '01000' SET MESSAGE_TEXT = 'Номер уже забронирован';
         ```
